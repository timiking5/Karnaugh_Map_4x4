...
vector<int> parse_rectangle_vertical(char **kmap, int k, int m, int n)
{
    ...
    if (j < 0)
        j = n - 1;
    vector<int> left_line = parse_line_vertical(kmap, j, m, n);
    vector<int> right_line = parse_line_vertical(kmap, i, m, n);
    if (mid_line.size() == 4 && right_line.size() == 4)
    {
        respond = vector<int>{k, i};
        return respond;
    }
    else if (mid_line.size() == 4 && left_line.size() == 4)
    {
        respond = vector<int>{j, k};
        return respond;
    }
    else
    {
...
vector<int> parse_rectangle_horizontal(char **kmap, int k, int m, int n)
{
    ...
    if (j < 0)
        j = n - 1;
    vector<int> top_line = parse_line_horizontal(kmap, k, j, n);
    vector<int> bottom_line = parse_line_horizontal(kmap, k, i, n);
    if (mid_line.size() == 4 && top_line.size() == 4)
    {
        respond = vector<int>{j, m};
        return respond;
    }
    else if (mid_line.size() == 4 && bottom_line.size() == 4)
    {
        respond = vector<int>{m, i};
        return respond;
    }
